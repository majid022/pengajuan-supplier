                            ...
                            <ul class="header-dropdown m-r--5">
                                 <li class="dropdown">
                                    <div class="button" style="margin-top: -17px">
                                        <button type="button" class="btn  bg-green btn-circle-lg waves-effect waves-circle waves-float" data-toggle="modal" onclick="tambah_project()"><i class="material-icons">add</i></button>
                                    </div>
                                </li>
                            </ul>
                        </div>
                                ...
                                <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td><?=$p->tgl?></td>
                                            <td align="center">
                                                <button type="button" onclick="edit(<?=$p->id_project?>)" class="btn bg-cyan btn-circle waves-effect waves-circle waves-float">
                                                   <i class="material-icons">edit</i>
                                               </button>
                                                <button type="button" onclick="hapus(<?=$p->id_project?>)" class="btn bg-red btn-circle waves-effect waves-circle waves-float">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="defaultModalLabel">Tambah Project</h4>
            </div>
            <div class="modal-body">
                <form action="#" id="form">
                    <label for="email_address">Nama Project</label>
                    <div class="form-group">
                        <div class="form-line">
                            <input type="hidden" name="id_project" class="form-control" >
                            <input type="text" name="nama_project" class="form-control" placeholder="Masukkan Nama Project">
                        </div>
                    </div>
                    <label for="password">Kode Project</label>
                    <div class="form-group">
                        <div class="form-line">
                            <input type="text" name="kode_project" class="form-control" placeholder="Masukkan Kode Project">
                        </div>
                    </div>
                </form>               
            </div>
   ...
   function tambah_project(){
    ...
    function simpan(){
        ...
        if(save_method == 'tambah'){
            url = "<?php echo base_url('admin/project/tambah')?>";
        }
        else
        {
            url = "<?php echo site_url('admin/project/update') ?>";
        }
     ...
     function edit(id_project)
    {
        ...
        $.ajax({
            url: "<?php echo base_url('admin/project/edit') ?>/" + id_project,
            type: "GET",
            dataType: "JSON",
            success: function (data)
            {
                ...
                $('[name="kode_project"]').val(data.kode_project);
    ...
    function hapus(id_project){
        swal({
                ...
                if(result.value){
                    $.ajax({
                    url:  "<?php echo site_url('admin/project/hapus') ?>",
                    type: "post",
                    data: {
